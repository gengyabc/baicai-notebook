...
- 知识摄取：从 `my-work/`、`resources/` 等来源自动分类、路由、提炼，默认直连 `wiki/`，不确定内容进入 `brainstorm/` 推演层
- 知识查询：按 `wiki -> resources -> brainstorm` 的顺序检索，并输出置信度与证据来源
- 推演整理：在 `brainstorm/todo` 与 `brainstorm/active` 中迭代想法，保持明确的不确定性标记
- 知识固化：通过唯一 `solidify` gate 将内容提升进入 `wiki/`，默认直连 wiki，不确定内容自动路由到 `brainstorm/`，设置时间元数据追踪时效性
...
- `/ingest my-work/decision.md` → 提炼要点 → 默认直连 `wiki/`，不确定内容路由到 `brainstorm/`
- `/ingest https://example.com/article` → 创建 `resources/` 笔记 → 依据证据质量直连 `wiki/` 或路由到 `brainstorm/`
...
- `/lint-vault` → 查 frontmatter、`image_key`、index entry、attachment locality、promotion candidates
...
这些情况下，内容先在 `brainstorm/` 中迭代，待稳定后再通过 `/solidify` 提升到 `wiki/`。

**从 my-work 或 resources 直连 wiki（默认）**

大部分内容会直接进入 `wiki/`：

- `/ingest my-work/decision.md` → 若内容稳定且有明确 provenance，直连 `wiki/`
- `/ingest Resources/paper.pdf` → 若论断清晰、证据充分，直连 `wiki/`

仍需经过 `solidify` gate，但无需先进入 `brainstorm/`。

**批量补齐 image_key**

使用 lint 发现缺失后，可手动补齐：

- `/lint-vault` → 标记 missing `image_key` 的笔记
- 后续批次补齐 frontmatter
...
|---------|---------|---------|---------|
| `/ingest` | `.opencode/commands/ingest.md` | 摄取 `my-work` 或 `resources` 内容进入 vault，自动判断直连 `wiki/` 或路由到 `brainstorm/` | 新笔记、外部来源、PDF、网页、Zotero 条目需要进入知识体系 |
| `/discover-requirements` | `.opencode/commands/discover-requirements.md` | 发现需求并在 ready 时写入 `docs/requirements/` | 需求澄清、范围界定、输出需求文档 |
| `/write_plan` | `.opencode/commands/write_plan.md` | 根据讨论结果生成 `docs/plans/` 计划文件 | 需要把讨论结果结构化为实施计划 |
| `/solidify` | `.opencode/commands/solidify.md` | 通过唯一 promotion gate 将内容提升到 `wiki/`，默认直连，不确定内容自动路由到 `brainstorm/`，设置时间元数据 | 确认内容适合稳定记忆层，需要自动路由决策与时效性追踪 |
| `/lint-vault` | `.opencode/commands/lint-vault.md` | 审计 vault 的结构、元数据、索引覆盖、附件局部性、promotion candidates | 定期维护、发现 drift、准备批量提升前检查健康度 |

---

## Workflows

Legacy routing diagrams are superseded by `docs/requirements/2026-04-14-system-rearchitecture.md` and `docs/plans/2026-04-14-system-rearchitecture.md`.

| Workflow | 文件路径 | 功能描述 | 使用场景 |
|----------|---------|---------|---------|
| `ingest-my-work` | `.opencode/workflows/ingest-my-work.md` | 从 `my-work/` 提炼要点，自动判断直连 `wiki/` 或路由到 `brainstorm/` | 处理用户笔记、决策、项目思考，保持用户意图 intact |
...
| `solidify-to-wiki` | `.opencode/workflows/solidify-to-wiki.md` | 作为唯一 promotion gate，检查 provenance、自动路由决策（直连 wiki 或路由到 brainstorm）、设置时间元数据，并维护 `wiki/index.md` 与 `wiki/log.md` | 需要将内容从 `my-work/` 或 `resources/` 提升到稳定记忆层，自动判断路由 |
...
| `promotion-policy` | `.opencode/rules/promotion-policy.md` | promotion 默认路径 `my-work/resources -> wiki -> output`，不确定内容路由到 `brainstorm/`；`solidify` 是唯一 gate；自动检测路由条件 | 任何 promotion 相关判断与流程 |
...
|-------|---------|---------|---------|
| `second-brain-ingest` | `.opencode/skills/second-brain-ingest/SKILL.md` | 分类与路由摄取内容，默认直连 `wiki/`，不确定内容进入 `brainstorm/`，`resources/` 渐进整理护栏，新建笔记含 `image_key` | workflow 调用，执行实际路由与笔记创建 |
...
| `solidify-to-wiki` | `.opencode/skills/solidify-to-wiki/SKILL.md` | 作为唯一 promotion gate，检查 provenance、自动路由决策（直连 wiki 或路由到 brainstorm），设置 `last_verified` 与 `review_cadence` 时间元数据，维护 `wiki/index.md` 与 `wiki/log.md` | workflow 调用，执行 wiki 提升、路由决策与索引维护 |
...
- 新建 `wiki/` 笔记时自动设置 `last_verified`（内容确认准确日期）和 `review_cadence`（复审频率）
- `review_cadence` 默认 `annual`，可按领域设置为 `quarterly` 或 `monthly`
- 当 wiki 笔记过期时，状态可标记为 `needs-review`，表示需要复审但非阻塞
- 现有笔记无需迁移，新内容仅适用新规则
...
```
my-work/resources -> wiki -> output
                   ↘ brainstorm (不确定内容)
```

- **默认直连 wiki**：内容清晰、证据充分、无冲突时直接提升
- **自动路由 brainstorm**：存在冲突、推测、综合不确定或新颖性时先在推演层迭代
- 进入 `wiki` 需要 explicit `solidify` gate
- 新 wiki 笔记包含 `last_verified` 和 `review_cadence` 时间元数据
- `review_cadence` 默认为 `annual`，可按领域调整
